...
def fetch():
    ...
    #recorro los datos desde directamente desde la query, no desde el cursor. Imprimo cada fila.
...
def search_by_grade(grade):
    ...
    print('Estudiantes que cursan el año "grade":\n')
    for fila in c.execute('SELECT id, name, age FROM estudiante WHERE grade = ?', (grade,)):
        print(fila)
...
if __name__ == '__main__':
    ...
    estudiantes = [('Chico', 17, 5, 'Dorival'), 
               ('Caetano', 16, 3, ''),
               ('Gal', 12, 1, 'Jorge' ),
               ('Gilberto', 17, 4, 'Baden'),
               ('Bethania', 16, 4, 'Vinicius'), 
               ('Tom', 17, 4, 'Joao')]
